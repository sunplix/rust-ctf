<template>
  <section class="page-block">
    <header class="page-head">
      <div>
        <p class="soft mono">TEAM WORKSPACE</p>
        <h1>队伍中心</h1>
        <p class="muted">创建队伍、邀请协作、管理成员并处理入队邀请。</p>
      </div>
      <RouterLink class="btn-ghost-link" to="/contests">返回比赛中心</RouterLink>
    </header>

    <p v-if="pageError" class="error">{{ pageError }}</p>

    <div class="teams-layout">
      <article class="panel teams-main">
        <div class="row-between">
          <h2>我的队伍</h2>
          <button class="btn-ghost" type="button" @click="refreshAll" :disabled="refreshing">
            {{ refreshing ? "刷新中..." : "刷新数据" }}
          </button>
        </div>
        ...
        <template v-else-if="myTeam">
          <section class="team-summary">
            ...
            <p class="soft mono">team_id={{ myTeam.id }}</p>
          </section>

          <div class="actions-row">
            <button class="btn-ghost" type="button" :disabled="leaveBusy" @click="handleLeaveTeam">
              {{ leaveBusy ? "处理中..." : "退出队伍" }}
            </button>
            <button
              v-if="isCaptain"
          ...
          <section v-if="isCaptain" class="team-card">
            <h3>队伍设置</h3>
            <form class="form-grid" @submit.prevent="handleUpdateTeam">
              ...
              <button class="btn" type="submit" :disabled="teamSettingBusy">
          ...
          <section v-if="isCaptain" class="team-card">
            ...
            <form class="form-grid" @submit.prevent="handleCreateInvitation">
              ...
              <button class="btn" type="submit" :disabled="inviteBusy">
                {{ inviteBusy ? "发送中..." : "发送邀请" }}
              </button>
            </form>
        ...
        <template v-else>
          <p class="muted">当前还没有队伍。你可以先创建，或通过名称加入已有队伍。</p>
          ...
          <section class="team-card">
            ...
            <form class="form-grid" @submit.prevent="handleCreateTeam">
              ...
              <button class="btn" type="submit" :disabled="actionBusy">
                {{ actionBusy ? "处理中..." : "创建队伍" }}
              </button>
            </form>
          </section>
          ...
          <section class="team-card">
            ...
            <form class="form-grid" @submit.prevent="handleJoinByName">
              ...
              <button class="btn-ghost" type="submit" :disabled="actionBusy">
                {{ actionBusy ? "处理中..." : "加入队伍" }}
              </button>
            </form>
          </section>
      ...
      <aside class="panel teams-side">
        <div class="row-between">
          <h2>队伍大厅</h2>
          <button class="btn-ghost" type="button" @click="loadTeams" :disabled="loadingTeams">
            {{ loadingTeams ? "加载中..." : "刷新列表" }}
          </button>
        </div>

        <form class="actions-row" @submit.prevent="loadTeams">
          <input v-model.trim="keyword" placeholder="搜索队伍名" />
          <button class="btn-ghost" type="submit" :disabled="loadingTeams">搜索</button>
        </form>
        ...
        <div v-else class="hall-list">
          <article v-for="team in teams" :key="team.id" class="hall-card">
            <div class="row-between">
            ...
            <p class="soft mono">team_id={{ team.id }}</p>
            <button
              class="btn"
              type="button"
